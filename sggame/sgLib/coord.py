...
class Coord(object):
    @staticmethod
    def PixelDistance(x,y,x2,y2):
        '''计算2坐标间的距离  像素坐标'''
        return int( (abs(x - x2)**2+abs(y - y2)**2)**0.5 )
        
    @staticmethod
    def Coord2Pixel(x,y):
        '''坐标转为格子 #@todo Pixel2Coord'''
    ...
    def ExpandCoord(x,y,xSize,ySize=None):
        ...
        if ySize:#任何矩形坐标
            return Coord.calCoords(x, y, xSize, ySize)
        else:#正方形坐标
            return Coord.ExpandSquare(x, y, xSize)
    ...
    def ExpandSquare(x,y,size):
        ...
        for i in xrange(1,size+1): #每行的格子数
            ...
            rd = size - i #与中间行的距离
            yt = y+rd
            xt = Coord.IncreaseX(x,y,rd) #将x向右上移动步数
            for j in xrange(i):
    ...
    def Get9Coord(x,y,space=1,isSelf=True): 
        ...
        #第一行
        CoordDict[1] = (x,y+iy)
        CoordDict[2] = (Coord.IncreaseX(x, y, ix),y+ix)
        CoordDict[3] = (x+ix,y)
        #第二行 
        CoordDict[4] = (Coord.IncreaseLeftX(x, y, ix),y+ix)
        CoordDict[5] = (x,y) #中心
        CoordDict[6] = (Coord.IncreaseLeftX(x, y, -ix),y-ix)
        #第三行
        CoordDict[7] = (x-ix,y)
        CoordDict[8] = (Coord.IncreaseX(x, y, -ix),y-ix)
        CoordDict[9] = (x,y-iy)
        ...
        if space == 1:
            #第一行
            CoordDict[1] = (x,y+8)
            CoordDict[2] = (Coord.IncreaseX(x, y, 4),y+4)
            CoordDict[3] = (x+4,y)
            #第二行 
            CoordDict[4] = (Coord.IncreaseLeftX(x, y, 4),y+4)
            CoordDict[5] = (x,y) #中心
            CoordDict[6] = (Coord.IncreaseLeftX(x, y, -4),y-4)
            #第三行
            CoordDict[7] = (x-4,y)
            CoordDict[8] = (Coord.IncreaseX(x, y, -4),y-4)
            CoordDict[9] = (x,y-8)
        elif space == 2:
            #第一行
            CoordDict[1] = (x,y+10)
            CoordDict[2] = (Coord.IncreaseX(x, y, 5),y+5)
            CoordDict[3] = (x+5,y)
            #第二行 
            CoordDict[4] = (Coord.IncreaseLeftX(x, y, 5),y+5)
            CoordDict[5] = (x,y) #中心
            CoordDict[6] = (Coord.IncreaseLeftX(x, y, -5),y-5)
            #第三行
            CoordDict[7] = (x-5,y)
            CoordDict[8] = (Coord.IncreaseX(x, y, -5),y-5)
            CoordDict[9] = (x,y-10)
        
        elif space == 3:
            #第一行
            CoordDict[1] = (x,y+12)
            CoordDict[2] = (Coord.IncreaseX(x, y, 6),y+6)
            CoordDict[3] = (x+6,y)
            #第二行 
            CoordDict[4] = (Coord.IncreaseLeftX(x, y, 6),y+6)
            CoordDict[5] = (x,y) #中心
            CoordDict[6] = (Coord.IncreaseLeftX(x, y, -6),y-6)
            #第三行
            CoordDict[7] = (x-6,y)
            CoordDict[8] = (Coord.IncreaseX(x, y, -6),y-6)
            CoordDict[9] = (x,y-12)
        '''
        return CoordDict
    ...
    def calCoords(x,y,xsize,ysize):
        ...
        '''
        coords = []
        xCoords = Coord.expandXY(x, y, xsize,'X')
        for x in xCoords:
            yCoords = Coord.expandXY(x[0],x[1],ysize,'Y')
            coords+=yCoords
        return coords
...
if __name__ == '__main__':
    '''调试'''
    print Coord.PixelDistance(100,200,100,200)