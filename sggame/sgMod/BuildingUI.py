...
class BuildingUI(object):
    ...
    def CreateBuilding(self, param={}):
        ...
        if BuildingType==1 or BuildingType==19 or BuildingType not in BuildingCfg:
            return Gcore.error(optId, -15001998) #将军府和城墙不可建造 或 建筑类型不正确
        ...
        #用户的所有建筑
        AllBuildings = self.mod.getBuildingById(fields='*', TimeStamp=TimeStamp)
        ...
        #建筑CD时间受内政4影响, 花费受内政7影响
        print '受内政影响前CD', CDValue
        print '受内政影响前花费', CostValue
        interMod = Gcore.getMod('Inter',self.uid)
        effect = interMod.getInterEffects({4:CDValue, 7:CostValue}, TimeStamp)
        CDValue = effect[4]
        CostValue = effect[7]
        print '受内政影响后CD', CDValue
        print '受内政影响后花费', CostValue
    ...
    def UpgradeBuilding(self, param={}):
        ...
        #建筑CD时间受内政4影响, 花费受内政7影响 
        interMod = Gcore.getMod('Inter',self.uid)
        effect = interMod.getInterEffects({4:CDValue, 7:CoinNum}, TimeStamp)
        CDValue = effect[4]
        CoinNum = effect[7] 
        print 'cd', CDValue
        print 'min', CDValue/60, 'sec', CDValue%60
        ...
        BuildingInfo['LastOptType'] = 1
        BuildingInfo['CompleteTime'] = TimeStamp + CDValue
        BuildingInfo['BuildingLevel'] = BuildingLevel + 1
        ...
        if BuildingType in (6, 8): #兵营 工坊：计算士兵
            ...
            modCamp.getSoldierStorage(BuildingInfo, TimeStamp, is_sync=True, set_cal_time=TimeStamp + CDValue)
...
def _test():
    ...
    #print c.UpgradeBuilding(p)
    ##print c.CancelBuilding({"BuildingId":658})
    print c.SpeedupProcess(p)
