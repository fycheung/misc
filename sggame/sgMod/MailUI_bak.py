...
def escapeSplit(str,sep=';'):
    '''
    @param string sep 分隔符
    '''
    strs = str.split('%s'%sep)
    return [n for n in strs if n]
...
#    data={'ToUserNickNames':'NickName1011','Subject':'你好毛泽东','Content':'SB 大 SB'}
#    print c.InsMessage(data)
    print c.InsMessage({'ToUserNickNames':'雷傲薇','Subject':'UI.SendMailTest','Content':'测试测试测试'})
#    print c.DelMail({'MailIds':[3544,3545],'MailType':1})
#    print c.MailList({'MailType':1,'Page':1})
#    print c.MailInfo({'MailType':1,'MailId':3315})

#    print c.ReceiveAttachments({'AttachIds':[3342,3343,3344]})
